...
public class QuestTreasureScene extends BaseScene {
    ...
    public QuestTreasureScene(final QuestResult questResult, final boolean isInvalidTreasure, final GameActivity activity) throws IOException {
        super(activity);
        this.questResult = questResult;
        this.isInvalidTreasure = isInvalidTreasure;
        init();
    }
    ...
    protected void init() throws IOException {
        ...
        if (isInvalidTreasure) {
            final Font titleFont = ResourceManager.getInstance().newFont(FontEnum.Default, 26);
            final Font detailFont = ResourceManager.getInstance().newFont(FontEnum.Default, 24);
            final TextOptions textOptions = new TextOptions(AutoWrap.LETTERS, 490);
            final Text descText = new Text(frame.getWidth() * 0.5f, 275, titleFont, MSG_INVALID_GUILD, vbom);
            descText.setColor(0XFF330504);
            frame.attachChild(descText);

            final Text dialogText = new Text(frame.getWidth() * 0.5f, 190, detailFont, MSG_INVALID_GUILD2, textOptions, vbom);
            dialogText.setColor(0XFF330504);
            frame.attachChild(dialogText);
        } else {
